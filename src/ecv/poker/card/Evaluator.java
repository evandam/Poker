...
public class Evaluator {
	...
	public static int evaluateFurther(List<Card> playerCards, int value) {
		...
		// evaluate a pair, then 3 kickers
			...
			for(int i = 1; i < cards.size(); i++) {
				if(cards.get(i).getRank() == cards.get(i-1).getRank()) {
					pair = cards.get(i).getRank();
					if(cards.size() > 2)
						kicker1 = cards.get((i + 1) % cards.size()).getRank();
					if(cards.size() > 3)
						kicker2 = cards.get((i + 2) % cards.size()).getRank();
					if(cards.size() > 4)
						kicker3 = cards.get((i + 3) % cards.size()).getRank();
				}
			}
		...
		// quads are first evaluated by the value of the 4 cards, then the last
			...
			int kicker = 0;
			if(cards.size() > 4) 
				kicker = cards.get((quadsIndex + 4) % cards.size()).getRank();
